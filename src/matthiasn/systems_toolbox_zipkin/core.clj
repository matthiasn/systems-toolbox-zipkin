(ns matthiasn.systems-toolbox-zipkin.core
...
(defn mk-reporter [base-url]
  (let [zipkin-url (str base-url "/api/v1/spans")
        sender (OkHttpSender/create zipkin-url)]
    (AsyncReporter/create sender)))
...
(defn trace-wrapper [f service-name reporter]
  ...
  (fn [cmp-map]
    (let [service-name (str (:cmp-id cmp-map))
          ...
          handler-map (->> (:handler-map cmp-map)
                           (map (fn [[k f]] [k (trace-wrapper f service-name reporter)]))
                           (into {}))]
